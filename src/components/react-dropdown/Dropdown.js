...
class Drowdown extends Component {
  constructor (props) {
    ...
    this.state = {
      ...
      selectAllListItem: {
        ...
        id: -1,
        value: 'select all',
        selected: props.items.filter(i => i.selected).length === props.items.length
      }
    }
  ...
  eachCheckBoxLi (item, index) {
    ...
    return (
      <li key={index} id={'item-' + item.id} className="list-item">
        <input className="styled-checkbox" onChange={this.onMultiSelectListItem.bind(this, item)} type="checkbox"
               id={'t-' + item.id} checked={item.selected}/>
        <div className={'styled-checkdiv' + classsuffix} />
        <label className= {'checkmark' + classsuffix} htmlFor={'t-' + item.id}>
          {item.name}
        </label>
      </li>
    )
    ...
    return (
      <li key={index} id={'item-' + item.id} className={item.selected ? 'list-item active' : 'list-item'}>
        <label className="radiomark" htmlFor={'t-' + item.id}>
          <input onChange={this.onSingleSelectListItem.bind(this, item)} name={'t1'} type="radio" id={'t-' + item.id}
                 checked={item.selected} style={{ visibility: 'hidden' }}/>
          {item.name}
        </label>
      </li>
    )
  ...
  onMultiSelectListItem (item, e) {
    this.setState(prev => ({
      items: prev.items.map(prevItem => prevItem.id === item.id ? {
        ...prevItem,
  ...
  showSelectedItemString () {
    ...
    if (mySelectedItems.length) {
      l = mySelectedItems.length
    }
    if (l === 1)
      return l + '  Type Selected'
    else
      return l + '  Types Selected'
  }

  renderMuliSelect () {
    let classsuffix = ''
    if (this.state.selectAllListItem.selected) {
      classsuffix = '-checked'
    }
    return (
      <ul className="mainUl">
        ...
        <li className="select-all list-item">
          <input onChange={this.onSelectAll} type="checkbox" className="styled-checkbox" id={'x-select-all'}
                 checked={this.state.selectAllListItem.selected}/>
          <div className={'styled-checkdiv' + classsuffix} />
